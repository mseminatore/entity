...
`benchmarks/entity_bench.cpp` is a manually-run microbenchmark suite (a small custom `bench::run` harness — min-of-5 wall-clock timing, not Google Benchmark), built as the `entity_bench` target. It's unconditionally forced to `-O2`/`/O2` regardless of the top-level build type, since an unoptimized build's numbers are meaningless (`benchmarks/CMakeLists.txt` also strips MSVC's `/RTC1` from `CMAKE_CXX_FLAGS_DEBUG` in that directory's scope first, since `/RTC1` and `/O2` are mutually exclusive and would otherwise fail a Debug-config build of just this target). Not registered with ctest — timing isn't a pass/fail signal. Run manually:
```
